...
class Abe:
    def __init__(abe, store, args):
        ...
        abe.log = logging.getLogger("__Aur-Wallet__")
    ...
    def handle_pushtx(abe, page):
        abe.do_raw(page, abe.do_pushtx)
    
    def handle_getexchangerate(abe, page):
        abe.do_raw(page, abe.do_getexchangerate)

    def handle_status(abe, page):
	abe.do_raw(page, abe.do_status)
    
    def do_status(abe, page):
	return "200"

    def do_getexchangerate(abe, page, chain):
	exchangerate = abe.store.selectrow("""
            SELECT rate 
	    FROM exchange_rate
	    WHERE firstcurrency = 'AUR'
	    AND secondcurrency = 'BTC'
	    ORDER BY datetime desc
		""")
	return exchangerate[0]

    def do_pushtx(abe, page, chain):
        #no chain is currently used for do_tx
        raw_tx = wsgiref.util.shift_path_info(page['env'])
        if raw_tx is None:
            return 'Pushes a transaction to the network.\n' \
                '/pushtx/RAW_TRANSACTION\n'
        else:
            #send the tx using rpc call to bitcoin
            #the variables are defined as per our server only
            #could read from the .conf file
	    try:
            	reply = bitcoin.sendrawtransaction(raw_tx)
		abe.log.critical('tx pushed: %s', reply)
	    	return 200
            except Exception:
		return 500
    ...
    def do_unspent(abe, page, chain):
        ...
        # RPC call for getmempool for fresh txids
        rawmempool = bitcoin.getrawmempool()
        vins = []
        vouts = []
        # RPC call for each txid to get json details  
        for tx_id in rawmempool:
            # RPC call for tx_id
            rawtxout = bitcoin.getrawtransaction(tx_id)
            txout = bitcoin.decoderawtransaction(rawtxout)
            # make list of rawmempool vins to eliminate from unspent
            for vin in txout['vin']:
                vins.append(vin['txid'])
            # make list of rawmempool vouts to add to unspent
            for vout in txout['vout']:
                if vout['scriptPubKey']['addresses'][0] == addrs[0]:
                    vouts.append({
                                'tx_hash': tx_id,
                                'tx_output_n': vout['n'],
        ...
        for row in rows:
            ...
            # remove each vin in mempool
            valid_tx = True
            for vin in vins:
            #if the unspent vin is in mempool invalidate txid
                if tx_hash == vin:
                    abe.log.warning("---mempool txid removed: %s: ", tx_hash)
                    valid_tx = False
            if valid_tx == True:
                # append each row to API output
                out.append({
                        'tx_hash': tx_hash,
                        'tx_output_n': out_pos,
                        'script': script,
        ...
        # append the fresh rawmempool txouts to the unspent API
        for row in vouts:
            #first check if vout has been used in a mempoolvin
            valid_tx = True
            for vin in vins:
                if row['tx_hash'] == vin:
                    valid_tx = False
                if valid_tx == True:
                    abe.log.warning("---mempool txid added: %s: ", row['tx_hash'])
                    out.append({
                    'tx_hash': row['tx_hash'],
                    'tx_output_n': row['tx_output_n'],
                            'script': row['script'],
                            'value': row['value'],
                            'value_hex': None,
                            'block_number': None})
    ...
    def q_addressbalance(abe, page, chain):
        ...
        version, hash = util.decode_address(addr)
        total = abe.store.get_balance(chain.id, hash, addr)
...
#function to create object of type ABE, initalize listener
def serve(store):
    #retrieve command line args (are there also url args query?)
    args = store.args
    #create object of type Abe
    abe = Abe(store, args)
    ...
    if args.query is not None:
        ...
        #use the simple server package from wsgiref.simple server
        from wsgiref.simple_server import make_server
        port = int(args.port or 80)
        #initialize server using make_server function
...
#---------------------------MAIN FUNCION OF PROGRAM--------------------------------------#---comand line args, configuration, system errors, chains, policies, database info
...
See abe.conf for commented examples.""")
    ...
    logger = logging.getLogger('__Aur-Wallet__')
    logger.setLevel(logging.DEBUG)
    # create file handler which logs even debug messages
    fh_debug = logging.FileHandler('/home/joseph/Documents/GitHub/bitcoin-abe/debug.log')
    fh_debug.setLevel(logging.DEBUG)
    # create file handler which logs only critical messages
    fh_crit = logging.FileHandler('/home/joseph/Documents/GitHub/bitcoin-abe/critical.log')
...
#MAKING NEW OBJECT STORE AND SEND ARGS


    #create new oject type DataStore
