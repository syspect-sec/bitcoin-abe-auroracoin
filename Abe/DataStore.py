...
class DataStore(object):
    ...
    def __init__(store, args):
        ...
        store.args = args
        store.log = logging.getLogger("__Aurora-Wallet__")
...
#This table seems to build a view for basic chain data
...
#this is a key table basically linking blocks to transactions
...
#    def _is_descended_from(store, block_id, ancestor_id):
    ...
    def get_balance(store, chain_id, pubkey_hash, addr):
        ...
        #RPC connection to AuroraCoind to get raw mempool and modify the balance return
        bitcoin_rpc = bitcoinrpc.authproxy.AuthServiceProxy(BITCOINRPC)
        vins = []
        vouts = []
        vins_database = []
        mempool_spent = 0
        mempool_received = 0
        rawmempool = bitcoin_rpc.getrawmempool()
        # RPC call for each txid to get json details for every txid in the rawmempool  
        for tx_id in rawmempool:
            rawtxout = bitcoin_rpc.getrawtransaction(tx_id)
            tx = bitcoin_rpc.decoderawtransaction(rawtxout)
            # add all vin spent for that address to spent again before the block is published
            for vin in tx['vin']:
                found_previous_output = False
                # look back in mempool for txid that matches the vin txid 
                for tx_id_prev in rawmempool:
                    if tx_id_prev == vin['txid']:
                        found_previous_output = True
                        store.log.info("vin found in mempool: %s", tx_id_prev)
                        # look through previous vouts for change value to address matching balance request
                        rawtxout_prev = bitcoin_rpc.getrawtransaction(tx_id_prev)
                        txout_prev = bitcoin_rpc.decoderawtransaction(rawtxout_prev)
                        for vout_prev in txout_prev['vout']:				
                        # if previous out of spent input matches address, add amount to spent
                            if vout_prev['scriptPubKey']['addresses'][0] == addr and vout_prev['n'] == vin['vout']:
                                # add amount to spent
                                store.log.info("Found spent mempool: %s", vout_prev['value']*100000000)
                                mempool_spent += int(vout_prev['value']*100000000)
                if found_previous_output == False:
                    store.log.info("vin added to db search: %s", vin['txid'])
                    # add txid to potential database search list
                    vins_database.append((vin['txid'], vin['vout']))

            #calculate all value in the rawmempool vout for balance requesting address
            for vout in tx['vout']:
                # look for the address that matches the balance call
                if vout['scriptPubKey']['addresses'][0] == addr:
                    mempool_received += int(vout['value']*100000000)

        # look in database 'abe' for vouts of vin txid that were not found in rawmempool and add to spent
        for tx_id_db in vins_database:
            # search txouts in database for address of requesting balance
            sql = """
            SELECT COALESCE(SUM(txout_value),0) 
            FROM txout
            JOIN pubkey ON
            pubkey.pubkey_id = txout.pubkey_id
            JOIN tx ON
            tx.tx_id = txout.tx_id
            WHERE pubkey.pubkey_hash = ? AND
            tx.tx_hash = ? AND
            txout.txout_pos = ?
            """
            dbhash = store.binin(pubkey_hash)
            store.log.info("dbhash: %s", dbhash)
            store.log.info("tx_id_db: %s", tx_id_db)
            # get amount and add to mempool_spent amount
            store.log.info("mempool_spent_database: %s", store.selectrow(sql, (dbhash, tx_id_db[0], tx_id_db[1]))[0])		
            mempool_spent += store.selectrow(sql, (dbhash, tx_id_db[0], tx_id_db[1]))[0]

        store.log.info("mempool_received: %s", mempool_received)
        store.log.info("mempool_spent: %s", mempool_spent)
        received += mempool_received
        sent += mempool_spent
        store.log.critical("Balance Returned for addr: %s %s"%(addr, str(received-sent)))
